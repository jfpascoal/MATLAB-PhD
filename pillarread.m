function [varargout]=pillarread(varargin)
%PILLAREAD
% PILLARREAD
...
% -- Image background subtraction
    function [newimg,totBkgrd,relBkgrd]=bkgrdSubt(img)
        ...
        if maxX>1  
            % After determining the optimal exponent, create the histogram
            % and find the threshold
            %             tempimg=double(uint16(img.^maxX));
            %             %create histogram n with x centers xHist
            %             [n,xHist]=hist(tempimg(tempimg>0),1000);
            %             %calculate derivate of n, turn all values positive and smooth
            %             dn=zeros(1,numel(n)-1);
            %             for i=1:numel(n)-1
            %                 dn(i)=(n(i+1)-n(i))/(xHist(i+1)-xHist(i));
            %             end
            %             posdn=smoother(smoother(sqrt(dn.^2),25),25);
            %
            %             % select the data relative to where the derivate is low
            %             % (i.e. under 2.5)
            %             logicslope=xHist(posdn<2.5);
            %
            %             if max(findpeaks(posdn))>posdn(1)
            %                 minthresh=max(xHist(posdn==max(findpeaks(posdn))));
            %             else
            %                 minthresh=xHist(1);
            %             end
            %
            %             logicslope=logicslope(logicslope>minthresh);
            %             minslopeX=logicslope(1);
            %             maskim=tempimg;
            %             for i=1:numel(img)
            %                 if maskim(i)<minslopeX
            %                     maskim(i)=0;
            %                 else
            %                     maskim(i)=1;
            %                 end
            ...
            % new code
            tempimg=double(uint16(img.^maxX));
            currImg=uint16(tempimg); dispImg
            
            % calculate density estimation
            % convert tempimg values >0 and <30001 to vector and sort
            fulldata=sort(reshape(tempimg(tempimg>0 & tempimg< 30001),...
                [],1));
            ...
            % Density estimation:
            % Create vector with all absolute differences between x values
            % under 10. The input of a maximum value is justified since the
            % higher the difference, the lower the product of the gaussian 
            % function will be.
            ...
            % Calculate the product of the kernel (gaussian) function for
            % all the differences.
            kernTable=zeros(dXSize,1); % pre-allocate
            for i=1:dXSize
                kernTable(i)=(1/sqrt(2^pi)*exp(-1/2*dX(i)^2))/...
                    numel(fulldata);
            end
            
            % Calculate the frequency of each pixel value in the image.
            freqTable=zeros(1,dataSize); % pre-allocate
            for i=1:xSize
                freqTable(i)=numel(fulldata(fulldata==x(i)));
            end
            
            % Calculate the total estimated density per x value by 
            % multiplying the kernel product for each relevant difference 
            % by the frequency the respective pixel value.
            ...
            % Note that because the bandwidth used for this estimation
            % (i.e. 1) is much lower than the difference between x points, 
            % the sum of all the density values will be much lower than 1.
            % Because the actual density values aren't relevant for this
            % analysis, this choice greatly increased the efficiency 
            % without sacrificing the results.
            
            % Differentiate and smooth density estimation
            ...
            % Find areas with small variations, i.e. dFD close to 0, after
            % the dFD max.
